...
def load_data(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logging.info('Data loaded Succeccfully from %s', file_path)
        return df
...
def apply_power_transformer(train_data: pd.DataFrame, test_data: pd.DataFrame) -> tuple:
    """Apply PowerTransformer (Yeo-Johnson) to normalize numerical features seperately for X_train and X_test."""
    try:
